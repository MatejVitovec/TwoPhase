...
#include "TwoFluidFlux.hpp"

Field<Vars<10>> TwoFluidFlux::calculateFluxes(const Field<TwoFluid>& ul, const Field<TwoFluid>& ur, const std::vector<Face>& faceList) const
{
    ...
    for (int i = 0; i < ul.size(); i++)
    {
        ...
        if (thetaGG > 0)
        {
            ggFlux = GGFluxSolver->calculateFlux(stateL.densityG(),         stateR.densityG(),
        ...
        if (thetaLL > 0)
        {
            llFlux = LLFluxSolver->calculateFlux(stateL.densityL(),         stateR.densityL(),
