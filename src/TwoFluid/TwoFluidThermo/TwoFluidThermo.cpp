...
void TwoFluidThermo::updateFromConservative(VolField<TwoFluid>& u, const Field<TwoFluidCompressible>& w, const Field<double>& pInt) const
{
    for (int i = 0; i < w.size(); i++)
    {
        ...
        const double thetaG = (0.5/w[i][TwoFluidCompressible::ALPHA_RHO_G])*(std::pow(w[i][TwoFluidCompressible::ALPHA_RHO_U_G], 2.0)
                                                                           + std::pow(w[i][TwoFluidCompressible::ALPHA_RHO_V_G], 2.0)
                                                                           + std::pow(w[i][TwoFluidCompressible::ALPHA_RHO_W_G], 2.0));

        const double thetaL = (0.5/w[i][TwoFluidCompressible::ALPHA_RHO_L])*(std::pow(w[i][TwoFluidCompressible::ALPHA_RHO_U_L], 2.0)
                                                                           + std::pow(w[i][TwoFluidCompressible::ALPHA_RHO_V_L], 2.0)
                                                                           + std::pow(w[i][TwoFluidCompressible::ALPHA_RHO_W_L], 2.0));
...
void TwoFluidThermo::updateInternal(Field<TwoFluid>& u) const
{
    for (int i = 0; i < u.size(); i++)
    {
        ...
        thermoDataRef[TwoFluid::RHO_G] = (pressure*gammaG)/((gammaG - 1.0)*cpG*u[i][TwoFluid::T_G]);
        thermoDataRef[TwoFluid::RHO_L] = ((pressure + pInfL)*gammaL)/((gammaL - 1.0)*cpL*u[i][TwoFluid::T_L]);
        thermoDataRef[TwoFluid::INT_E_G] = (cpG/gammaG)*u[i][TwoFluid::T_G];
        thermoDataRef[TwoFluid::INT_E_L] = (cpL/gammaL)*u[i][TwoFluid::T_L] + pInfL/thermoDataRef[TwoFluid::RHO_L];
...
void TwoFluidThermo::updateBoundary(VolField<TwoFluid>& u) const
{
    for (int id = 0; id < u.boundarySize(); id++)
    {
        for (int i = 0; i < u.boundary(id).size(); i++)
        {
            ...
            thermoDataRef[TwoFluid::RHO_G] = (pressure*gammaG)/((gammaG - 1.0)*cpG*u.boundary(id)[i][TwoFluid::T_G]);
            thermoDataRef[TwoFluid::RHO_L] = ((pressure + pInfL)*gammaL)/((gammaL - 1.0)*cpL*u.boundary(id)[i][TwoFluid::T_L]);
            thermoDataRef[TwoFluid::INT_E_G] = (cpG/gammaG)*u.boundary(id)[i][TwoFluid::T_G];
            thermoDataRef[TwoFluid::INT_E_L] = (cpL/gammaL)*u.boundary(id)[i][TwoFluid::T_L] + pInfL/thermoDataRef[TwoFluid::RHO_L];
            thermoDataRef[TwoFluid::A_G] = std::sqrt((gammaG*pressure)/thermoDataRef[TwoFluid::RHO_G]);
            thermoDataRef[TwoFluid::A_L] = std::sqrt((gammaL*(pressure + pInfL))/thermoDataRef[TwoFluid::RHO_L]);
        }
    }    
