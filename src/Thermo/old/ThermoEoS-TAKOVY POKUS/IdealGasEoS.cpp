...
double IdealGasEoS::pressure(const Compressible& data) const
{
    return (gamma - 1.0)*data.density()*((data.totalEnergy() - 0.5*data.absVelocity2() - energyShift)); //pridan posun energie
}

double IdealGasEoS::soundSpeed(const Compressible& data) const
{
    return std::sqrt((gamma*std::max(0.0, pressure(data)))/data[Compressible::RHO]);
}

double IdealGasEoS::temperature(const Compressible& data) const
{
    return pressure(data)/(R*data[Compressible::RHO]);
}

Vars<3> IdealGasEoS::updateThermo(const Compressible& data, const ThermoVar& thermoData) const
{
...
Compressible IdealGasEoS::primitiveToConservative(const Vars<5>& primitive) const
{
...
Compressible IdealGasEoS::stagnationState(double TTot, double pTot) const
{
...
Compressible IdealGasEoS::isentropicInlet(double pTot, double TTot, double rhoTot, double sTot, double hTot, Vars<3> velocityDirection, Compressible stateIn, ThermoVar thermoIn) const
{
...
std::array<double, 3> IdealGasEoS::initPressureTemperatureInlet(double pTot, double TTot) const
{
